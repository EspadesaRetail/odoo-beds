$(document).ready(function () {
    ...
    $('.oe_website_sale').each(function () {
        ...
        /* Permite indicar si hay que recoger el colchón anterior */
        $(oe_website_sale).on("change", 'input[name="remove_old_products"]', function (event) {
            var remove_old_products = $(event.target).is(":checked");
            openerp.jsonRpc("/shop/set_remove_old_products", 'call', {'remove_old_products': remove_old_products})
                    .then(function (data) {
            ...
            openerp.jsonRpc("/shop/get_unit_price", 'call', {
                ...
                'line_id': line_id})
                    .then(function (res) {
                        ...
                        /*
                         * lo quito, ya que al actualizar la cantidad, se actualizaban los precios individuales de las líneas.
                         *
                        //basic case
                        ...
                        openerp.jsonRpc("/shop/cart/update_json", 'call', {
                            ...
                            'set_qty': value})
                                .then(function (data) {
                                    ...
                                    // Si hay recogida de usados, refrescar la pantalla.
                                    window.location.reload();
        ...
        // Visualizar la imagen correspondiente:
        $('.css_attribute_color input', oe_website_sale).on('click', function (ev) {
            var $color = $(this);
            var value = $color.attr('value');
            $("[attribute-value="+value+"]").trigger( "click" );
        ...
        function price_to_str(price) {
            ...
            //return price.toLocaleString('es-ES', {'maximumFractionDigits':2});

            return price.toLocaleString('es-ES', {'maximumFractionDigits': 2, 'minimumFractionDigits': 2});
        }

        function discount_to_str(discount) {
        ...
        $(oe_website_sale).on('change', 'input.js_variant_change, select.js_variant_change, ul[data-attribute_value_ids]', function (ev) {
            ...
            // Locallizar los divs, que contienen datos que dependen del descuento.
            var $discount = $parent.find(".sel-product-discount");
            var $antes =    $parent.find(".cs-product-lst-price");
            //var $discount = $parent.find(".sel-product-discount");
            ...
            for (var k in variant_ids) {
                if (_.isEmpty(_.difference(variant_ids[k][1], values))) {
                    ...
                    var t = discount_to_str(variant_ids[k][4]);

                    if(variant_ids[k][5] > 0) {
                        t = t + "+" + discount_to_str(variant_ids[k][5])
                        $discount.removeClass("pr-product-discount");
                        $discount.addClass("pr-product-plan-renove");
                    } else {
                        $discount.addClass("pr-product-discount");
                        $discount.removeClass("pr-product-plan-renove");
                    }
