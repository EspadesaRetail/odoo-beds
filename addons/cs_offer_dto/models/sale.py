...
class sale_order_line(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if not chk_recursion:
            for sol in self.browse(cr, uid, [new_id], context=context):
                sol._dto_update_order_with_promo()
    ...
    def _dto_update_order_with_promo(self):
        ...
        for offer_id in line_id.product_id.active_product_offer_ids:
            # Campaña sin iva.
            if offer_id.offer_type == 'dto':
                lst_price = self.product_id.lst_price * (1 - offer_id.additional_dto / 100)
                price = self.product_id.price * (1 - offer_id.additional_dto / 100)
                line_id.with_context(chk_recursion = True ).write({'price_unit': price, 'lst_price': lst_price})
